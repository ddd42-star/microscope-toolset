...
class SoftwareEngeneeringAgent:
    ...
    def write_script(self, fileName: str, context: str, microscope_status: str, previous_outputs: str, new_strategy: str, query: str) -> str:

        if len(context) == 0:
            context = "no information"

        prompt = SOFTWARE_PROMPT.format(filename=fileName,context=context, microscope_status=microscope_status, previous_outputs=previous_outputs, new_strategy=new_strategy)
    ...
    def run_code(self, fileName: str, context: str, microscope_status: str, previous_outputs: str, new_strategy: str, query: str, executor: Execute):
        # format generated code
        code_generated = self.write_script(fileName=fileName, context=context, microscope_status=microscope_status, previous_outputs=previous_outputs, new_strategy=new_strategy, query=query)
