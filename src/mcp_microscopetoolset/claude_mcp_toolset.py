...
@mcp_server.tool(
    name="Write the name",
    description="Write the description of the tool. Specify well input and output and how it should behave."
)
async def name_of_the_tool(parameters="Here the parameter to give"):

    return None

# Tool for handling feedback
@mcp_server.tool(
    name="handle_user_request",
    description="To do"
)
async def handle_user_request(user_query: str):
    # Initiate session context
    session_context = SessionContext(user_query)
    # add other parts
    try:
        server_answer = mcp_server.call_tool(name="process_user_request", arguments="example")
